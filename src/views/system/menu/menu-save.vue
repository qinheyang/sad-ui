<template>
  <el-drawer :title="dialogFormTitle" :visible.sync="dialogFormVisible" class="save-form-div"
    :close-on-click-modal="$globeValue.clickModalClose" @closed="closed()">
    <el-form v-loading="loading" :model="saveForm" :rules="rules" ref="saveForm" label-width="100px" size="medium">
      <el-form-item prop="menuName" label="菜单名">
        <el-input placeholder="请输入菜单名" v-model="saveForm.menuName" maxlength="50" auto-complete="off">
        </el-input>
      </el-form-item>
      <el-form-item prop="parentName" label="上级菜单">
        <tree-select ref="treeSelect" :data="menuTree" :treeSelectProps="treeSelectPorps"
          @getSelectTreeData="getSelectTreeData">
        </tree-select>
      </el-form-item>
      <el-form-item prop="menuType" label="菜单类型">
        <el-radio-group v-model="saveForm.menuType">
          <el-radio v-for="(m, i) in menuTypeArray" :key="i" :label="m.value">{{
            m.label
          }}</el-radio>
        </el-radio-group>
      </el-form-item>
      <el-form-item v-show="saveForm.menuType != menuTypeArray[2].value" prop="icon" label="图标">
        <icon-select ref="iconSelect" @getIcon="getIcon"></icon-select>
      </el-form-item>
      <el-form-item prop="path" label="路由地址" v-show="saveForm.menuType != menuTypeArray[2].value">
        <el-input placeholder="请输入路由地址" v-model="saveForm.path" maxlength="200" auto-complete="off" />
      </el-form-item>
      <el-form-item prop="isFrame" label="是否是外链" v-show="saveForm.menuType != menuTypeArray[2].value">
        <el-radio-group v-model="saveForm.isFrame">
          <el-radio v-for="(m, i) in isFrameArray" :key="i" :label="m.value">{{
            m.label
          }}</el-radio>
        </el-radio-group>
      </el-form-item>
      <el-form-item prop="component" label="组件地址" v-show="saveForm.menuType == menuTypeArray[1].value">
        <el-input placeholder="请输入路由地址" v-model="saveForm.component" maxlength="200" auto-complete="off" />
      </el-form-item>
    </el-form>
